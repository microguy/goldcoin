...
void OnRPCStarted()
{
    uiInterface.AddNotifyBlockTipCallback(RPCNotifyBlockChange);
}
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-checkpointdepth", _("Set the depth in the chain when a synced checkpoint will be set and broadcast if -checkpointkey is set"));
        strUsage += HelpMessageOpt("-checkpointkey", _("Set the synced checkpoint private key and broadcast checkpoints with -checkpointdepth"));
    ...
    strUsage += HelpMessageOpt("-rpcserialversion", strprintf(_("Sets the serialization of raw transaction or block hex returned in non-verbose mode, currently enforced to non-segwit (%d)"), DEFAULT_RPC_SERIALIZE_VERSION));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.goldcoinproject.org>";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    PSETPROCDEPPOL setProcDEPPol = (PSETPROCDEPPOL)(void*)GetProcAddress(GetModuleHandleA("Kernel32.dll"), "SetProcessDEPPolicy");
...
bool AppInitMain(thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    auto serviceLoop = [&scheduler]() { scheduler.serviceQueue(); };
    threadGroup.create_thread([serviceLoop]() { TraceThread("scheduler", serviceLoop); });
    ...
    {
        uiInterface.AddInitMessageCallback(SetRPCWarmupStatus);
    ...
    if (chainActive.Tip() == nullptr) {
        uiInterface.AddNotifyBlockTipCallback(BlockNotifyGenesisWait);
    } else {
        fHaveGenesis = true;
    }

    if (IsArgSet("-blocknotify"))
        uiInterface.AddNotifyBlockTipCallback(BlockNotifyCallback);
