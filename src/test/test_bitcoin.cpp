...
#define BOOST_TEST_MODULE Goldcoin Test Suite
...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = GetTempPath() / strprintf("test_goldcoin_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
        fs::create_directories(pathTemp);  // C++23 filesystem
...
TestingSetup::~TestingSetup()
{
        ...
        delete pcoinsdbview;
        delete pblocktree;
        fs::remove_all(pathTemp);  // C++23 filesystem
}
