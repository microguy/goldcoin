...
const std::string strMessageMagic = "Goldcoin (GLC) Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        int64_t nSigOpsCost = GetTransactionSigOpCost(tx, view, getSTANDARD_SCRIPT_VERIFY_FLAGS());
        ...
        unsigned int scriptVerifyFlags = getSTANDARD_SCRIPT_VERIFY_FLAGS();
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        // GetTransactionSigOpCost counts 2 types of sigops:
        // * legacy (always)
...
bool RewindBlockIndex(const CChainParams& params)
{
    ...
    for (BlockMap::iterator it = mapBlockIndex.begin(); it != mapBlockIndex.end(); it++) {
        ...
        if (false /*IsWitnessEnabled(pindexIter->pprev, params.GetConsensus()) && !(pindexIter->nStatus & BLOCK_OPT_WITNESS) && !chainActive.Contains(pindexIter)*/) {
            // Reduce validity
